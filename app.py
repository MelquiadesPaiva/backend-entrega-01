...
def create():
    ...
    if email not unique:#caso tentem colocar um email ja existente
        return{'error':'email ja existente'}    
...
def put_users(id):
    ...
    if id in users:
         for name, idade, email in req.items():
             data.insert('id',name)  #vai atualizar tudo, mesmo que nao seja requisitado
             data.insert('id',email)
             data.insert('id',idade)

            users(name=name,idade=idade,email=email)
...
def patch_users_db(id):               
    
    req = request.get_json()

    #atualiza ou cria uma colecao

    name = data.get('name')
    email = data.get('email')
    idade = data.get('idade')

     if id in users:     #vai checar se realmente existe o id no db
        for name, idade, email in req.items():#atualiza somente o requisitado
            if data.insert('id',email) #vai pegar o objeto no id e atauliza-lo/ NA ENTREGA PEDE PARA ATUALIZAR O EMAIL
                users(name=name,idade=idade,email=email)
...
@app.route('users/<int:id>', methods=['DELETE']) #deleta um termo
def delete_id(id):

    if id in users:

        del users[id]
            res = make_response(jsonify({'mensagem':'id deletado'}), 200) #quando o id existe e consegue ser apagado
            return res

    else:

        res = make_response(jsonify({'error': 'id nao encontrado'}), 400 ) #quando nao consegue achar o id
        return res
